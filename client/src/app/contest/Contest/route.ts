...
export async function POST(req: NextRequest) {
  try {
    ...
    const prom = testcases.map((t) => {
      ...
      return axios
        .post<{ token: string }>(judgeurl, submissionBody)
        .then(({ data }) => {
          tokens.push(data.token);
        })
        .catch((err: any) => {
          throw new Error(
            err.message ? err.message : "Unable to reach Judge0."
          );
        });
    });
    ...
    try {
      ...
      const usersubmission = await UserProblem.findOneAndUpdate(
        { user: decodedtoken.id },
        { $inc: { submission: 1 } }
      );
      if (!usersubmission) {
      ...
      if (!updated) {
        let updated = await Participant.findOneAndUpdate(
          { userId: decodedtoken.id, contestId },
          {
            $push: {
              problems: {
                alias,
      ...
      const activity = await Activity.findOneAndUpdate(
        { user: decodedtoken.id, "activity.date": targetDate },
        { $inc: { "activity.$.submissions": 1 } }
      );
      if (!activity) {
    ...
    return NextResponse.json(
      {
        success: true,
        tokens,
        message: "successfully sent to run codes.",
