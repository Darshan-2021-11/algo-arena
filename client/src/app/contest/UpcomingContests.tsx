    ...
    useEffect(() => {
        const fetchContests = async () => {
            try {
                ...
                const url = `Api/Contests/GetContests?t=upcoming`;
    ...
    return (
        <>
            ...
            {loading ? (
                ...
                <ul className="flex transition-all mb-4" >
                    {contests.length === 0 && <div>No upcoming contests.</div> }
                    {contests.map((contest) => (
                        <Link key={uuidv4()} href={`/contest/${contest._id}`} >
                            <li key={uuidv4()} className="bg-gray-500 bg-opacity-35 w-40 box-content hover:bg-opacity-55 hover:m-2 transition-all p-4 rounded shadow-md m-1">
