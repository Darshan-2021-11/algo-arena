...
const OngoingContests = () => {
    ...
    useEffect(() => {
        const fetchContests = async () => {
            try {
                ...
                const url = `Api/Contests/GetContests?t=ongoing`;
    ...
    return (
        <>
            <h1 className="text-2xl font-bold mb-4 ">Ongoing contests</h1>
            <div className=" overflow-scroll h-44" ref={elemref}>
            {loading ? (
                <p>Loading contests...</p>
            ) : (
                <ul className="flex transition-all" >
                    {contests.length === 0 && <div>No ongoing contests.</div> }
                    {contests.map((contest) => (
                        <Link key={uuidv4()} href={`/contest/${contest._id}`} >
                            <li key={uuidv4()} className="bg-gray-500 bg-opacity-35 w-40 hover:bg-opacity-55 hover:m-2 transition-all p-4 rounded shadow-md m-1">
